...
def nomenc():
    ...
    if request.method == "POST":
        ...
        # 5' Red and 3' Green
        elif TEST_NAME == 'MYC' or TEST_NAME == 'BCL2' or TEST_NAME == 'BCL6' or TEST_NAME == 'RET' or TEST_NAME == 'EWSR1' or TEST_NAME == 'SYT' or TEST_NAME == "GENE_RED":
            for i in range(df.shape[0]):
                if df['Red'].loc[i] > 0 and df['Green'].loc[i] > 0 and df['Yellow'].loc[i] >= 0:
                    split += 1  # adding 1 to the split case
                elif df['Red'].loc[i] == 0 and df['Green'].loc[i] == 0 and df['Yellow'].loc[i] > 0:
        ...
        if TEST_NAME == 'ROS1':
            ...
            cut = 'A EWSR1 rearrangement is reported if more than 15% of cells show split signals.'
        elif TEST_NAME == 'FKHR':
            ...
            cut = 'A sample is classified as negative if <15% of nuclei (7 nuclei or fewer) are positive for gene rearrangement and classified as positive if >15% (8 nuclei or more) are positive. If more than 50 nuclei are enumerated, a sample is classified as positive if ≥15% of nuclei are positive.'
        elif TEST_NAME == 'GENE_RED':
            cut = 'A GENE_RED rearrangement is reported if more than 15% of cells show split signals.'
        elif TEST_NAME == 'GENE_GREEN':
            cut = 'A GENE_GREEN rearrangement is reported if more than 15% of cells show split signals.'
        else:
            cut = ''
    ...
    return render_template('nom.html', Nom=Nom, out_final=out_final, TEST_NAME=TEST_NAME, add=add, nom1=nom1, nom2=nom2,
                           ...
                           non_fused=non_fused, fsd=fsd, splt=splt, iso3=iso3, iso5=iso5)
