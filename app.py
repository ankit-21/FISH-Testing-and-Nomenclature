...
def fish_sankey(
    ...
    iso5_green = ['ROS1', 'ALK', 'CHOP', 'NTRK1', 'NTRK3', 'GENE_GREEN']
...
def nomenc():
    ...
    if request.method == "POST":
        ...
        iso5_green = ['ROS1', 'ALK', 'CHOP', 'NTRK1', 'NTRK3', 'GENE_GREEN']
        iso5_red = ['MYC', 'BCL2', 'BCL6', 'FKHR', 'RET', 'EWSR1', 'SYT', 'GENE_RED']
        ...
        if TEST_NAME in iso5_green:
            ...
            try:
                num_1 = sum(tot1) / len(tot1)
                if decimalPoint(num_1) >= 5:
                    num1 = int(num_1) + 1
                else:
                    num1 = int(num_1)
            ...
            try:
                num_1_f = sum(tot2) / len(tot2)
                if decimalPoint(num_1_f) >= 5:
                    num1_f = int(num_1_f) + 1
                else:
                    num1_f = int(num_1_f)
            except ZeroDivisionError:
                num1_f = 0
            ...
            try:
                num_2 = sum(tol) / len(tol)
                if decimalPoint(num_2) >= 5:
                    num2 = int(num_2) + 1
                else:
                    num2 = int(num_2)
            except ZeroDivisionError:
            ...
            try:
                num_3 = sum(tot_red1) / len(tot_red1)
                if decimalPoint(num_3) >= 5:
                    num3 = int(num_3) + 1
                else:
                    num3 = int(num_3)
            except ZeroDivisionError:
            ...
            try:
                num_4 = sum(tot_green1) / len(tot_green1)
                if decimalPoint(num_4) >= 5:
                    num4 = int(num_4) + 1
                else:
                    num4 = int(num_4)
            except ZeroDivisionError:
            ...
            try:
                num_5 = sum(tot_red2) / len(tot_red2)
                if decimalPoint(num_5) >= 5:
                    num5 = int(num_5) + 1
                else:
                    num5 = int(num_5)
            except ZeroDivisionError:
            ...
            try:
                num_6 = sum(tot_green2) / len(tot_green2)
                if decimalPoint(num_6) >= 5:
                    num6 = int(num_6) + 1
                else:
                    num6 = int(num_6)
            except ZeroDivisionError:
            ...
            # Calculating the avg of yellow column for iso 5' & 3'
            yellow_avg_red = []
            yellow_avg_green = []
            for i in range(df.shape[0]):
                if df['Red'].loc[i] > 0 and df['Green'].loc[i] == 0:
                    yellow_avg_red.append(df.Yellow.loc[i])
                elif df['Red'].loc[i] == 0 and df['Green'].loc[i] > 0:
                    yellow_avg_green.append(df.Yellow.loc[i])

            try:
                num_red_1 = sum(yellow_avg_red) / len(yellow_avg_red)
                if decimalPoint(num_red_1) >= 5:
                    num_red = int(num_red_1) + 1
                else:
                    num_red = int(num_red_1)
            except ZeroDivisionError:
                num_red = 0

            try:
                num_green_1 = sum(yellow_avg_green) / len(yellow_avg_green)
                if decimalPoint(num_green_1) >= 5:
                    num_green = int(num_green_1) + 1
                else:
                    num_green = int(num_green_1)
            except ZeroDivisionError:
                num_green = 0
            ...
            " Isolated 5': nuc ish (5' TEST_NAME x avg(G), 3' TEST_NAME x avg(R))(5' TEST_NAME con 3' TEST_NAME x avg(Y)) [iso 5'/total] "
            " Isolated 3': nuc ish (5' TEST_NAME x avg(G), 3' TEST_NAME x avg(R))(5' TEST_NAME con 3' TEST_NAME x avg(Y)) [iso 3'/total] "
            ...
            if isolated5 > 0:
                iso5 = " (5' " + TEST_NAME + " x " + str(int(num4)) + ', ' + "3' " + TEST_NAME + " x " + str(
                    int(num3)) + ")" + "(5' " + TEST_NAME + " con 3' " + TEST_NAME + " x " + str(int(num_green)) + ")" + " [" + "<b>" + str(isolated5) + "</b>" + "/" + str(add) + "] "  # isolated 5'
            else:
                iso5 = ""

            if isolated3 > 0:
                iso3 = " (5' " + TEST_NAME + " x " + str(int(num6)) + ', ' + "3' " + TEST_NAME + " x " + str(
                    int(num5)) + ")" + "(5' " + TEST_NAME + " con 3' " + TEST_NAME + " x " + str(int(num_red)) + ")"  + " [" + "<b>" + str(isolated3) + "</b>" + "/" + str(add) + "] "  # isolated 3'
            else:
                iso3 = ""
        ...
        # 5' Red and 3' Green
            ...
            for i in range(df.shape[0]):
                if df['Red'].loc[i] == 0 and df['Green'].loc[i] == 0 and df['Yellow'].loc[i] > 0:
                    tot2.append(df['Yellow'].loc[i] + (df['Red'].loc[i] + df['Green'].loc[i]) / 2)
            try:
                num1_f_1 = sum(tot2) / len(tot2)
                if decimalPoint(num1_f_1) >= 5:
                    num1_f = int(num1_f_1) + 1
                else:
                    num1_f = int(num1_f_1)
            except ZeroDivisionError:
                num1_f = 0
            ...
            " Split: nuc ish (TEST_NAME x [{(R+G)/2} + Y]) (5' TEST_NAME sep 3' x min(R,G)) [split/total] "
            " Fused: nuc ish (TEST_NAME x [{(R+G)/2} + Y]) [fused/total] "
            " Isolated 5': nuc ish (5' TEST_NAME x avg(R), 3' TEST_NAME x avg(G))(5' TEST_NAME con 3' TEST_NAME x avg(Y)) [iso 5'/total] "
            " Isolated 3': nuc ish (5' TEST_NAME x avg(R), 3' TEST_NAME x avg(G))(5' TEST_NAME con 3' TEST_NAME x avg(Y)) [iso 3'/total] "
            ...
            if isolated5 > 0:
                iso5 = " (5' " + TEST_NAME + " x " + str(int(num3)) + ', ' + "3' " + TEST_NAME + " x " + str(
                    int(num4)) + ")" + "(5' " + TEST_NAME + " con 3' " + TEST_NAME + " x " + str(int(num_red)) + ")" + " [" + "<b>" + str(isolated5) + "</b>" + "/" + str(add) + "] "  # isolated 5'
            else:
                iso5 = ""

            if isolated3 > 0:
                iso3 = " (5' " + TEST_NAME + " x " + str(int(num5)) + ', ' + "3' " + TEST_NAME + " x " + str(
                    int(num6)) + ")" + "(5' " + TEST_NAME + " con 3' " + TEST_NAME + " x " + str(int(num_green)) + ")" + " [" + "<b>" + str(isolated3) + "</b>" + "/" + str(add) + "] "  # isolated 3'
            else:
                iso3 = ""
